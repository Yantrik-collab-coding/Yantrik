...
export default function ProjectPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/projects/${id}`).then(r => {
      ...
      // In Electron: ensure ~/Yantrik/{projectName}/ exists and store path as terminal CWD
    ...
    // Cleanup: stop file watcher on unmount
    return () => {
      const api2 = (window as any).electronAPI
      if (api2?.isDesktop && project?.name) {
        api2.stopFileWatcher(project.name)
      }
    }
  }, [id])
  ...
  async function saveFile() {
    ...
    // In Electron: keep local disk in sync with manual saves too
  ...
  async function deleteFile(f: WorkspaceFile) {
    ...
    // In Electron: remove from local disk too
    const eAPI = (window as any).electronAPI
    if (eAPI?.isDesktop && project?.name) {
  ...
  // Sync files changed/created/deleted by external tools (npm, git, terminal)
  useEffect(() => {
    const eAPI = (window as any).electronAPI
    ...
    const offCreated = eAPI.onFileCreated((data: { projectId: string; filename: string; content: string }) => {
      ...
      api.post(`/projects/${id}/sync-file`, { filename: data.filename, content: data.content })
        .then(() => {
          console.log('[FileWatcher] Synced new file:', data.filename)
          loadFiles()
          // Show notification
          setMessages(prev => [...prev, {
            id: `fs-${Date.now()}`,
            author_name: 'System',
            content: `File created: ${data.filename}`,
            is_agent: false,
    ...
    const offChanged = eAPI.onFileChanged((data: { projectId: string; filename: string; content: string }) => {
      if (data.projectId !== id) return
      // Sync changed file to database
      api.post(`/projects/${id}/sync-file`, { filename: data.filename, content: data.content })
        .then((r: any) => {
          if (r.data.action !== 'unchanged') {
            console.log('[FileWatcher] Synced file change:', data.filename)
    ...
    const offDeleted = eAPI.onFileDeleted((data: { projectId: string; filename: string }) => {
      ...
      api.delete(`/projects/${id}/sync-file?filename=${encodeURIComponent(data.filename)}`)
        .then(() => {
          console.log('[FileWatcher] Synced file deletion:', data.filename)
          loadFiles()
  ...
  async function acceptDiff(diffId: string) {
    ...
    try {
      ...
      // In Electron: mirror the accepted file to ~/Yantrik/{projectName}/
      const eAPI = (window as any).electronAPI
      if (eAPI?.isDesktop && project?.name) {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Terminal panel — real PTY in Electron, run-output fallback in browser */}
        {showTerminal && (
          <Terminal
            projectId={id!}

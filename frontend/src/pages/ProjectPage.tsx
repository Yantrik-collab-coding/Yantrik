...
import {
  ...
  FilePlus, FolderPlus, File, Trash2, RotateCcw, ChevronDown, X, History, Download
} from 'lucide-react'
...
/** Lightweight markdown renderer for agent chat responses */
...
export default function ProjectPage() {
  ...
  const [modelsSource, setModelsSource]   = useState<'live' | 'static'>('static')
  ...
  const [triggerRootCreate, setTriggerRootCreate] = useState<{ type: 'file' | 'folder' } | null>(null)
  ...
  const sidebar  = useResize({ defaultSize: SIDEBAR_DEF,  min: SIDEBAR_MIN,  max: SIDEBAR_MAX,  direction: 'horizontal', storageKey: 'yantrik-sidebar-w'  })
  const filetree = useResize({ defaultSize: FILETREE_DEF, min: FILETREE_MIN, max: FILETREE_MAX, direction: 'horizontal', storageKey: 'yantrik-filetree-w',
    onResize: () => monacoRef.current?.layout()
  })
  const chat     = useResize({ defaultSize: CHAT_DEF,     min: CHAT_MIN,     max: CHAT_MAX,     direction: 'horizontal', storageKey: 'yantrik-chat-w',
    invert: true,  // chat panel is on the RIGHT, grows leftward
    onResize: () => monacoRef.current?.layout()
  })
  const terminal = useResize({ defaultSize: TERMINAL_DEF, min: TERMINAL_MIN, max: TERMINAL_MAX, direction: 'vertical',   storageKey: 'yantrik-terminal-h', invert: true  })
  const diffPanel = useResize({ defaultSize: DIFF_DEF,    min: DIFF_MIN,     max: DIFF_MAX,     direction: 'vertical',   storageKey: 'yantrik-diff-h'      })

  // ── Fetch models (live from provider APIs, fallback to static) ─────────────

  async function fetchModels() {
    setModelsLoading(true)
    try {
      const r = await api.get('/projects/models/live')
      setModels(r.data)
      const hasLive = r.data.some((m: any) => m.source === 'live')
      setModelsSource(hasLive ? 'live' : 'static')
    } catch {
      // fallback to static
      try {
        const r = await api.get('/projects/models')
        setModels(r.data)
  ...
  useEffect(() => {
    ...
    api.get(`/projects/${id}`).then(r => {
      setProject(r.data)
      setMyModel(r.data.my_model)
      const api2 = (window as any).electronAPI
      if (api2?.isDesktop) {
        api2.ensureProjectDir(r.data.name).then((res: any) => {
          if (res.ok) setProjectCwd(res.path)
        })
        api2.startFileWatcher(r.data.name, id).then((res: any) => {
          if (res.ok) console.log('[ProjectPage] File watcher started for', r.data.name)
        })
      }
    }).catch(() => navigate('/'))
    fetchModels()
    ...
    return () => {
      const api2 = (window as any).electronAPI
      if (api2?.isDesktop && project?.name) api2.stopFileWatcher(project.name)
    }
  }, [id])

  // Auto-open terminal in Electron Desktop mode
  useEffect(() => {
    if (projectCwd && (window as any).electronAPI?.isDesktop) {
      setShowTerminal(true)
    }
  }, [projectCwd])

  // ── Electron native menu event handlers ────────────────────────────────────
  useEffect(() => {
    const eApi = (window as any).electronAPI
    if (!eApi?.onMenuEvent) return

    const cleanups = [
      eApi.onMenuEvent('save',            () => { if (activeFile && id) saveFile() }),
      eApi.onMenuEvent('save-all',        () => { if (activeFile && id) saveFile() }),
      eApi.onMenuEvent('toggle-terminal', () => setShowTerminal((p: boolean) => !p)),
      eApi.onMenuEvent('open-settings',   () => navigate('/profile')),
      eApi.onMenuEvent('open-profile',    () => navigate('/profile')),
      eApi.onMenuEvent('new-project',     () => navigate('/dashboard')),
      eApi.onMenuEvent('open-file', (data: { filename: string; content: string; path: string }) => {
        if (!id) return
        api.post(`/projects/${id}/files`, { filename: data.filename, content: data.content })
          .then(() => loadFiles())
          .catch(() => {})
      }),
      eApi.onMenuEvent('export-zip', async (data: { savePath: string }) => {
        if (!id) return
        try {
          const response = await api.get(`/projects/${id}/download`, { responseType: 'arraybuffer' })
          await eApi.writeRawFile(data.savePath, new Uint8Array(response.data))
        } catch {}
      }),
      eApi.onMenuEvent('find', () => {
        monacoRef.current?.getAction?.('actions.find')?.run()
      }),
      eApi.onMenuEvent('open-folder', async (data: any) => {
        if (data.action === 'new-project') {
          navigate('/dashboard', { state: { prefillName: data.name, prefillPath: data.path } })
        } else if (data.action === 'import-files' && data.files?.length) {
          // Batch import all files
          for (const f of data.files) {
            try {
              await api.post(`/projects/${id}/files`, { filename: f.filename, content: f.content })
            } catch {}
          }
          loadFiles()
        }
  ...
  async function runFile() {
    ...
    setRunning(true); setShowTerminal(true); setRunOutput(null)
    try {
  ...
  async function loadFiles() {
    ...
    // ── Sync all files to disk so terminal can use them ──────────────
    const eAPI = (window as any).electronAPI
    if (eAPI?.syncWriteAllFiles && project?.name) {
      // Need full content for each file — fetch individually for files missing content
      const filesWithContent = await Promise.all(
        fileList.map(async (f: any) => {
          if (f.content !== undefined) return f
          try {
            const fr = await api.get(`/projects/${id}/files/${f.id}`)
            return { ...f, content: fr.data.content }
          } catch { return f }
        })
      )
      eAPI.syncWriteAllFiles(project.name, filesWithContent).catch(() => {})
    }
  }
  ...
  async function saveFile() {
    ...
    setUnsaved(false)
    const eAPI = (window as any).electronAPI
    if (eAPI?.isDesktop && project?.name) {
      eAPI.writeFile(project.name, activeFile.filename, activeContent).catch((err: any) => {
        console.warn('[file-sync] Failed to write file to disk:', err)
      })
    }
  }
  ...
  async function deleteFile(f: WorkspaceFile) {
    ...
    const eAPI = (window as any).electronAPI
    if (eAPI?.isDesktop && project?.name) {
      eAPI.deleteFile(project.name, f.filename).catch((err: any) => {
        console.warn('[file-sync] Failed to delete file from disk:', err)
      })
    }
  }
  ...
  // ── File Watcher (Electron only) ────────────────────────────────────────────
  useEffect(() => {
    const eAPI = (window as any).electronAPI
    if (!eAPI?.isDesktop || !id) return

    const offCreated = eAPI.onFileCreated((data: { projectId: string; filename: string; content: string }) => {
      if (data.projectId !== id) return
      api.post(`/projects/${id}/sync-file`, { filename: data.filename, content: data.content })
        .then(() => {
          loadFiles()
          setMessages(prev => [...prev, {
            id: `fs-${Date.now()}`, author_name: 'System',
            content: `File created: ${data.filename}`, is_agent: false,
            timestamp: new Date().toISOString(), type: 'system'
          }])
        }).catch((err: any) => console.error('[FileWatcher] Failed to sync new file:', err))
    })

    const offChanged = eAPI.onFileChanged((data: { projectId: string; filename: string; content: string }) => {
      if (data.projectId !== id) return
      api.post(`/projects/${id}/sync-file`, { filename: data.filename, content: data.content })
        .then((r: any) => {
          if (r.data.action !== 'unchanged') {
            if (activeFile?.filename === data.filename && !unsaved) {
              loadFiles().then(() => {
                const updatedFile = files.find(f => f.filename === data.filename)
                if (updatedFile) openFile(updatedFile)
              })
            } else { loadFiles() }
          }
        }).catch((err: any) => console.error('[FileWatcher] Failed to sync file change:', err))
    })

    const offDeleted = eAPI.onFileDeleted((data: { projectId: string; filename: string }) => {
      if (data.projectId !== id) return
      api.delete(`/projects/${id}/sync-file?filename=${encodeURIComponent(data.filename)}`)
        .then(() => {
          loadFiles()
          if (activeFile?.filename === data.filename) { setActiveFile(null); setActiveContent('') }
        }).catch((err: any) => console.error('[FileWatcher] Failed to sync file deletion:', err))
    })

    return () => { offCreated(); offChanged(); offDeleted() }
  }, [id, activeFile?.filename, unsaved])

  function handleWsMessage(data: any) {
    const ts = data.timestamp || new Date().toISOString()

    if (data.type === 'agent_typing') { setAgentTyping({ username: data.username, model: data.model }); return }
    if (data.type === 'message') { setAgentTyping(null); setMessages(prev => [...prev, { ...data, timestamp: ts }]); return }
    if (data.type === 'system') {
    ...
    if (data.type === 'agent_thinking') {
      setMessages(prev => [...prev, { id: `thinking-${data.username}-${Date.now()}`, author_name: `${data.username}'s Agent`, content: data.message, is_agent: true, timestamp: ts, type: 'thinking' }]); return
    }
    if (data.type === 'agent_plan') {
      const steps: Step[] = data.steps.map((s: any) => ({ step_number: s.step_number, action: s.action, target_file: s.target_file, status: 'pending' }))
    ...
    if (data.type === 'agent_step_start') {
      setActiveJobs(prev => {
        const job = prev[data.job_id]; if (!job) return prev
    ...
    if (data.type === 'agent_step_done') {
      ...
      // Refresh file tree whenever a create/modify step completes
      if (data.status === 'done' && data.action !== 'explain') {
        loadFiles()
        // ── Sync the specific file to disk immediately ────────────────
        const eAPI = (window as any).electronAPI
        if (eAPI?.writeFile && project?.name && data.target_file && data.new_content) {
          eAPI.writeFile(project.name, data.target_file, data.new_content).catch(() => {})
        }
      }
    ...
    if (data.type === 'agent_job_done') {
      loadFiles()          // Final sync — ensure all agent-created files appear
      loadPendingDiffs()   // Sync diffs too
    ...
    if (data.type === 'file_accepted') {
      ...
      if (activeFile?.filename === data.filename) { setActiveContent(data.content || ''); setUnsaved(false) }; return
    }
    if (data.type === 'file_rejected') { loadPendingDiffs(); return }
    if (data.type === 'memory_update') {
      // Upsert YANTRIK_MEMORY.md in local files state so the editor reflects it
      setFiles(prev => {
        const exists = prev.find(f => f.filename === data.filename)
        if (exists) return prev.map(f => f.filename === data.filename ? { ...f, content: data.content } : f)
        return [...prev, { id: `memory-${Date.now()}`, filename: data.filename, content: data.content, language: 'markdown' }]
      })
      return
    }
    if (data.type === 'agent_no_key') { setNoKeyBanner({ provider: data.provider, key_url: data.key_url, model: data.model }); return }
    if (data.type === 'agent_error') {
      setMessages(prev => [...prev, { id: `err-${Date.now()}`, author_name: 'System', content: data.error, is_agent: false, timestamp: ts, type: 'agent_error' }]); return
    }
  }
  ...
  async function downloadProject() {
    ...
    const res = await fetch(`/api/projects/${id}/download`, { headers: { Authorization: `Bearer ${token}` } })
    if (!res.ok) return
  ...
  async function acceptDiff(diffId: string) {
    ...
    try {
      ...
      const eAPI = (window as any).electronAPI
      if (eAPI?.isDesktop && project?.name) {
        eAPI.writeFile(project.name, r.data.filename, r.data.content).catch((err: any) => {
          console.warn('[file-sync] Failed to write file to disk:', err)
        })
      }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg)' }}>
      ...
      <div style={{ width: sidebar.size, background: 'var(--bg1)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
          ...
          <div style={{ padding: 12, overflow: 'auto', flex: 1 }}>
            ...
            <div style={{ marginTop: 16, borderTop: '1px solid var(--border)', paddingTop: 12 }}>
              <div style={{
                ...
                color: 'var(--text-dim)', fontSize: 11, marginBottom: 6
              }}>
                <span><Bot size={11} style={{ display: 'inline', marginRight: 4 }} /> MY MODEL</span>
                <button
                  onClick={fetchModels}
                  ...
                  style={{
                    ...
                    color: 'var(--text-dim)', padding: 2, opacity: modelsLoading ? 0.5 : 1,
                    fontSize: 11,
                  }}
                >
                  {modelsLoading ? '⏳' : '↻'}
              ...
              <select value={myModel} onChange={e => updateModel(e.target.value)} style={{ width: '100%', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 6, padding: '6px 8px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--font-mono)', outline: 'none', cursor: 'pointer' }}>
                {(['groq', 'openai', 'anthropic', 'google', 'ollama'] as const).map(provider => {
                  const providerModels = models.filter(m => m.provider === provider && m.available)
                  if (providerModels.length === 0) return null
                  const labels: Record<string, string> = { groq: '⚡ Groq (Fast)', openai: '🟢 OpenAI', anthropic: '🟣 Anthropic', google: '🔵 Google', ollama: '🖥️ Ollama (Local)' }
                  return (
                    <optgroup key={provider} label={labels[provider]}>
                      {providerModels.map(m => <option key={m.id} value={m.id}>{m.name}{(m as any).source === 'live' ? ' ✓' : ''}</option>)}
                    </optgroup>
                  )
                })}
              </select>
              <div style={{ fontSize: 10, color: 'var(--text-dim)', marginTop: 4, fontFamily: 'var(--font-mono)' }}>
                {modelsLoading
                  ...
                  : modelsSource === 'live'
                    ...
                    : 'Add API keys in Profile to unlock more models'
                }
              </div>
            </div>
          </div>
      ...
      <ResizeHandle direction="horizontal" onMouseDown={sidebar.handleMouseDown} />
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: 'var(--bg1)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          ...
          <button className="btn btn-icon btn-ghost btn-sm" title="New file" onClick={() => setTriggerRootCreate({ type: 'file' })}><FilePlus size={13} /></button>
          <button className="btn btn-icon btn-ghost btn-sm" title="New folder" onClick={() => setTriggerRootCreate({ type: 'folder' })}><FolderPlus size={13} /></button>
        </div>
        ...
        {fileLimits && (
          <div style={{ padding: '6px 12px', background: 'var(--bg1)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
              ...
              {/* DISABLED — testing/friends phase: no upgrade prompts */}
              {false && fileLimits.tier === 'free' && (
        ...
        {/* File tree + Editor row */}
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>

          {/* File tree */}
          <div style={{ width: filetree.size, background: 'var(--bg1)', borderRight: '1px solid var(--border)', overflow: 'auto', flexShrink: 0 }}>
            <FileTree
              files={files}
              activeFileId={activeFile?.id}
              pendingDiffFilenames={pendingDiffs.map(d => d.filename)}
              onOpenFile={openFile}
              onDeleteFile={deleteFile}
              onCreateFile={async (path) => {
                if (!id) return
                await api.post(`/projects/${id}/files`, { filename: path, content: '' })
                loadFiles()
              }}
              onCreateFolder={async (path) => {
                if (!id) return
                await api.post(`/projects/${id}/folders`, { folder_path: path })
                loadFiles()
              }}
              onRenameFile={async (file, newName) => {
                if (!id) return
                await api.patch(`/projects/${id}/files/${file.id}/rename`, { new_filename: newName })
                loadFiles()
                if (activeFile?.id === file.id) {
                  setActiveFile(prev => prev ? { ...prev, filename: newName } : prev)
                }
              }}
              triggerRootCreate={triggerRootCreate}
              onRootCreateDone={() => setTriggerRootCreate(null)}
              renamingFileId={renamingFileId}
              onRenameDone={() => setRenamingFileId(null)}
            />
          </div>

          {/* Resize handle: file tree | editor */}
          <ResizeHandle direction="horizontal" onMouseDown={filetree.handleMouseDown} />
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
            {activeFile ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', background: 'var(--bg2)', borderBottom: '1px solid var(--border)', flexShrink: 0, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                    ...
                    <button
                      ...
                      onClick={runFile} disabled={running} title="Run file"
                    >
                    ...
                    <button
                      ...
                      onClick={() => setShowTerminal(p => !p)} title="Toggle terminal"
                    >⬛ Terminal</button>
                  </div>
                </div>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-dim)', gap: 12 }}>
                ...
                <button
                  style={{ marginTop: 8, padding: '6px 14px', fontSize: 11, background: showTerminal ? 'var(--bg3)' : 'var(--bg)', color: 'var(--text-muted)', border: '1px solid var(--border)', borderRadius: 5, cursor: 'pointer', fontFamily: 'var(--font-display)', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 5 }}
                  onClick={() => setShowTerminal(p => !p)} title="Toggle terminal"
                >⬛ {showTerminal ? 'Hide' : 'Open'} Terminal</button>
              </div>
            )}
          </div>
        </div>

        {/* Diff panel — resizable vertical */}
        {showDiffPanel && pendingDiffs.length > 0 && (
          <>
            <ResizeHandle direction="vertical" onMouseDown={diffPanel.handleMouseDown} />
            <div style={{ height: diffPanel.size, background: 'var(--bg1)', overflow: 'auto', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', padding: '8px 12px', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, background: 'var(--bg1)', zIndex: 1 }}>
        ...
        {/* Terminal panel — resizable vertical */}
        {showTerminal && (
          <>
            <ResizeHandle direction="vertical" onMouseDown={terminal.handleMouseDown} />
            <div style={{ height: terminal.size, flexShrink: 0, overflow: 'hidden' }}>
              <Terminal
                projectId={id!}
      ...
      {/* ── Resize handle: middle | chat ─────────────────────────────────────── */}
      <ResizeHandle direction="horizontal" onMouseDown={chat.handleMouseDown} />

      {/* ── Right: Chat panel ────────────────────────────────────────────────── */}
      <div style={{ width: chat.size, background: 'var(--bg1)', display: 'flex', flexDirection: 'column', flexShrink: 0, overflow: 'hidden' }}>
        ...
        {noKeyBanner && (
          <div style={{ background: '#f0a50015', borderBottom: '1px solid #f0a50040', padding: '10px 14px', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
              ...
              <button onClick={() => setNoKeyBanner(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-dim)', padding: 0, flexShrink: 0 }}>
                <X size={13} />
              </button>
            </div>
          </div>
              ...
              <div key={msg.id} style={{ display: 'flex', gap: 8 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 13, lineHeight: 1.55, color: 'var(--text)', whiteSpace: 'pre-wrap', wordBreak: 'break-word', ...(msg.is_agent ? { background: 'var(--bg2)', border: '1px solid #bc8cff22', borderRadius: 7, padding: '8px 12px' } : {}) }}>
                    {msg.is_agent ? renderMarkdown(msg.content) : msg.content}
                  </div>
                </div>
              </div>
            )

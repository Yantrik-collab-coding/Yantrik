...
export default function ForumPage() {
  ...
  useEffect(() => {
    load()
    const interval = setInterval(load, 10000) // refresh every 10s for live counts
    ...
    <div style={S.root}>
      ...
      <div style={S.header}>
        ...
        <button className="btn btn-primary" onClick={() => setShowCreate(true)}>
          <Plus size={15} /> New Public Project
        </button>
      </div>
      ...
      <div style={S.filterBar}>
        ...
        <div style={S.tags}>
          <button
            style={{ ...S.tagBtn, ...(activeTag === '' ? S.tagBtnActive : {}) }}
            onClick={() => setActiveTag('')}
          >All</button>
          {allTags.map(tag => (
            <button
              key={tag}
              style={{ ...S.tagBtn, ...(activeTag === tag ? S.tagBtnActive : {}) }}
        ...
        <div style={S.overlay} onClick={() => setShowCreate(false)}>
          <div className="card" style={S.modal} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
              <Globe size={18} color="var(--accent2)" />
              <h2 style={{ fontSize: 18, fontWeight: 700 }}>New Public Project</h2>
            </div>
            <form onSubmit={createPublic} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ background: 'var(--accent2-dim)', border: '1px solid var(--accent2)33', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: 'var(--text-muted)' }}>
    ...
    <div style={S.card} onClick={onClick} className="forum-card">
      <div style={S.cardTop}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ ...S.projectIcon, background: p.owner_color + '33', color: p.owner_color }}>
        ...
        {isActive && (
          <div style={S.activeChip}>
            <span style={{ ...S.liveDot, background: 'var(--green)' }} />
            LIVE
          </div>
        )}
      </div>
      ...
      <div style={S.cardStats}>
        <span style={S.stat}><Users size={12} /> {p.member_count}</span>
        <span style={S.stat}><Eye size={12} /> {p.viewer_count}</span>
        <span style={S.stat}><MessageSquare size={12} /> {p.message_count}</span>
        <span style={S.stat}><Bot size={12} /> {p.agent_count}</span>
        <span style={{ ...S.stat, marginLeft: 'auto', color: 'var(--text-dim)' }}>
          <Clock size={11} /> {timeAgo(p.last_activity)}
        </span>
      </div>
    </div>
  )
...
const S: Record<string, React.CSSProperties> = {
  root: { minHeight: '100vh', background: 'var(--bg)', padding: '32px 40px' },
  ...
  backBtn: { background: 'none', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', fontSize: 13, fontFamily: 'var(--font-mono)', textAlign: 'left', padding: 0, marginBottom: 4 },
  titleRow: { display: 'flex', alignItems: 'center', gap: 10 },
  title: { fontSize: 30, fontWeight: 800, letterSpacing: '-0.02em' },
  liveChip: { display: 'flex', alignItems: 'center', gap: 5, background: '#3fb95022', border: '1px solid #3fb95044', borderRadius: 20, padding: '3px 10px', fontSize: 10, fontWeight: 700, color: 'var(--green)', fontFamily: 'var(--font-mono)' },
  liveDot: { width: 6, height: 6, borderRadius: '50%', background: 'var(--green)', display: 'inline-block', animation: 'pulse 2s infinite' },
  subtitle: { color: 'var(--text-muted)', fontSize: 14 },
  filterBar: { display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 },
  searchWrap: { position: 'relative', maxWidth: 400 },
  tags: { display: 'flex', flexWrap: 'wrap', gap: 8 },
  tagBtn: { background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 20, padding: '4px 12px', fontSize: 12, color: 'var(--text-muted)', cursor: 'pointer', fontFamily: 'var(--font-display)', transition: 'all 0.15s' },
  tagBtnActive: { background: 'var(--accent2-dim)', borderColor: 'var(--accent2)', color: 'var(--accent2)' },
  ...
  projectName: { fontWeight: 700, fontSize: 15, letterSpacing: '-0.01em' },
  ownerName: { fontSize: 12, color: 'var(--text-muted)', marginTop: 2 },
  activeChip: { display: 'flex', alignItems: 'center', gap: 4, background: '#3fb95022', border: '1px solid #3fb95044', borderRadius: 20, padding: '3px 8px', fontSize: 10, fontWeight: 700, color: 'var(--green)', fontFamily: 'var(--font-mono)', flexShrink: 0 },
  desc: { fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' } as any,
  cardStats: { display: 'flex', alignItems: 'center', gap: 14, borderTop: '1px solid var(--border)', paddingTop: 10, marginTop: 2 },
  stat: { display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: 'var(--text-muted)' },
  overlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 },
  modal: { width: '100%', maxWidth: 500, padding: 28 },

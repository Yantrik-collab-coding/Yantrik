...
async def delete_project(project_id: str, current=Depends(get_current_user), db=Depends(get_db)):
    ...
    await db.execute("DELETE FROM file_versions    WHERE file_id IN (SELECT id FROM workspace_files WHERE project_id=?)", (project_id,))
    await db.execute("DELETE FROM workspace_files   WHERE project_id=?", (project_id,))
    await db.execute("DELETE FROM pending_diffs     WHERE project_id=?", (project_id,))
    await db.execute("DELETE FROM agent_jobs        WHERE project_id=?", (project_id,))
    ...
    await db.execute("DELETE FROM join_requests     WHERE project_id=?", (project_id,))
    await db.execute("DELETE FROM project_viewers   WHERE project_id=?", (project_id,))

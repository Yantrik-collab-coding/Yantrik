...
async def _get_user_tier(user_id: str, db) -> str:
    return "ollama"  # Testing phase — all users get max limits
...
# ── Model endpoints (registered first to avoid /{project_id} shadowing) ──────
...
async def get_live_models(current=Depends(get_current_user), db=Depends(get_db)):
    """
    Returns only models the user can actually USE right now —
    by querying each provider API they have keys for.
    Falls back to static list if fetch fails.
    """
    import os
    import httpx
    from core.models import MODEL_DISPLAY
    from core.encryption import decrypt

    user_id = current["sub"]
    available_models = []

    # Fetch all API keys for this user (encrypted)
    async with db.execute(
        "SELECT provider, key_value FROM user_api_keys WHERE user_id=?",
        (user_id,)
    ) as cur:
    ...
    for row in key_rows:
        try:
            keys[row["provider"]] = decrypt(row["key_value"])
        except Exception:
            keys[row["provider"]] = row["key_value"]
    ...
    if groq_key:
        try:
            ...
            for mid, info in MODEL_DISPLAY.items():
                if info["provider"] == "groq":
                    available_models.append({
                        "id": mid, "name": info["name"],
                        "provider": "groq", "available": bool(groq_key),
                        "source": "static"
                    })
    ...
    if openai_key:
        try:
            async with httpx.AsyncClient(timeout=5) as client:
                ...
                if r.status_code == 200:
                    known_openai = {k for k, v in MODEL_DISPLAY.items() if v["provider"] == "openai"}
                    live_ids = {m["id"] for m in r.json().get("data", [])}
                    for mid in known_openai:
                        available_models.append({
                            "id": mid,
                            "name": MODEL_DISPLAY[mid]["name"],
                            "provider": "openai",
                            "available": mid in live_ids,
                            "source": "live"
                        })
        except Exception:
            for mid, info in MODEL_DISPLAY.items():
    ...
    # ── Ollama ───────────────────────────────────────────────────
    ollama_url = keys.get("ollama_url", "http://localhost:11434")
    try:
...
# ── Folder creation (virtual — via .gitkeep placeholder) ──────────────────────

class CreateFolderBody(BaseModel):
    folder_path: str  # e.g. "backend" or "frontend/src"

@router.post("/{project_id}/folders")
async def create_folder(project_id: str, body: CreateFolderBody,
                        current=Depends(get_current_user), db=Depends(get_db)):
    """
    Folders are virtual in Yantrik — they exist as long as files inside them exist.
    Creating a folder inserts a .gitkeep placeholder file so the folder appears
    immediately in the tree even before files are added to it.
    """
    import posixpath
    await _assert_member(project_id, current["sub"], db)
    folder = body.folder_path.strip().replace("\\", "/").strip("/")
    if not folder:
        raise HTTPException(status_code=400, detail="Folder path required")
    placeholder = posixpath.join(folder, ".gitkeep")
    file_id = str(uuid.uuid4())
    async with db.execute(
        "SELECT id FROM workspace_files WHERE project_id=? AND filename=?",
    ...
    if not exists:
        await db.execute(
            ...
            (file_id, project_id, placeholder, "", "plaintext", current["sub"])
        )
        await db.commit()
    return {"ok": True, "folder": folder, "placeholder_file": placeholder}
...
@router.patch("/{project_id}/files/{file_id}/rename")
async def rename_file(project_id: str, file_id: str, body: RenameBody,
                      current=Depends(get_current_user), db=Depends(get_db)):
    import posixpath
    await _assert_member(project_id, current["sub"], db)
    new_name = body.new_filename.strip().replace("\\", "/").lstrip("/")
    new_name = posixpath.normpath(new_name).lstrip("/")
    now = datetime.utcnow().isoformat()
    await db.execute(
        "UPDATE workspace_files SET filename=?, updated_at=? WHERE id=? AND project_id=?",
        (new_name, now, file_id, project_id)
    )
    await db.commit()
...
async def create_file(project_id: str, body: CreateFileRequest, current=Depends(get_current_user), db=Depends(get_db)):
    ...
    # Normalize: replace backslashes, strip leading slash, collapse double slashes
    import posixpath
    clean = body.filename.strip().replace("\\", "/").lstrip("/")
    clean = posixpath.normpath(clean).lstrip("/")  # removes ../  traversal
...
# ── Terminal (web fallback — run arbitrary commands) ───────────────────────────

class TerminalCommand(BaseModel):
    command: str

@router.post("/{project_id}/terminal")
async def run_terminal_command(project_id: str, body: TerminalCommand, current=Depends(get_current_user), db=Depends(get_db)):
    """Run a shell command in a sandboxed subprocess. Web fallback for terminal."""
    await _assert_member(project_id, current["sub"], db)
    ...
    if any(b in cmd.lower() for b in blocked):
        return {"stdout": "", "stderr": "⚠️ This command is blocked for safety.", "exit_code": 1}
    ...
    try:
        ...
        return {"stdout": "", "stderr": "⏱ Command timed out (15s limit)", "exit_code": 1}
    except FileNotFoundError:
        return {"stdout": "", "stderr": f"Command not found: {cmd.split()[0]}", "exit_code": 127}
    except Exception as e:
        return {"stdout": "", "stderr": str(e), "exit_code": 1}

...
from routers.billing import router as billing_router  # Stub — payments disabled, key management active
...
# NOTE: files_router must be registered BEFORE projects.router since both share
# the /api/projects prefix. Files has more specific routes (e.g. /folders, /files)
# that need to be matched first to avoid 405 from the generic projects routes.
app.include_router(auth.router,         prefix="/api/auth",       tags=["auth"])
app.include_router(files_router,        prefix="/api/projects",   tags=["files"])
app.include_router(projects.router,     prefix="/api/projects",   tags=["projects"])
app.include_router(chat.router,         prefix="/api/chat",       tags=["chat"])
app.include_router(billing_router,      prefix="/api/billing",    tags=["billing"])  # Stub — payments disabled, key management active

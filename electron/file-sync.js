...
// ── Bulk sync: write all project files to disk at once ────────────────────────

ipcMain.handle('file-sync:write-all', (event, { projectName, files }) => {
  const errors = []
  const dir = ensureProjectDir(projectName)
  for (const file of files) {
    try {
      if (file.filename === 'YANTRIK_MEMORY.md') continue  // skip internal files
      if (file.filename.endsWith('.gitkeep')) continue       // skip placeholder files
      const filePath = resolveFilePath(dir, file.filename)
      fs.mkdirSync(path.dirname(filePath), { recursive: true })
  ...
  console.log(`[file-sync] Bulk wrote ${files.length - errors.length} files to ${dir}`)
  if (errors.length) console.warn('[file-sync] Bulk write errors:', errors)

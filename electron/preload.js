...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  /**
   * Bulk-write all project files to disk at once.
   * Called on project open to ensure terminal can access all files.
   * @returns {Promise<{ok: boolean, errors?: Array}>}
   */
  syncWriteAllFiles: (projectName, files) =>
    ipcRenderer.invoke('file-sync:write-all', { projectName, files }),
  ...
  /**
   * Subscribe to a native menu event.
   * @param {string} event - Event name (e.g. 'save', 'toggle-terminal', 'open-file')
   * @param {Function} callback - Called with optional data payload
   * @returns {() => void} cleanup function
   */
  ...
  /**
   * Write raw bytes to a file on disk (for ZIP export etc.)
   * @param {string} filePath - Absolute path to write to
   * @param {Uint8Array} buffer - File contents
   * @returns {Promise<{ok: boolean, error?: string}>}
   */
  writeRawFile: (filePath, buffer) =>
    ipcRenderer.invoke('file:write-raw', { filePath, buffer }),

  ...
  const template = [
          ...
          click: async () => {
            ...
            if (!result.canceled && result.filePaths.length > 0) {
              ...
              const choice = await dialog.showMessageBox(mainWindow, {
                type: 'question',
                title: 'Open Folder',
                message: `Open "${folderName}"`,
                detail: 'How would you like to open this folder?',
                buttons: [
                  'Create New Project from Folder',
                  'Import Files into Current Project',
                  'Cancel'
                ],
                defaultId: 0,
                cancelId: 2,
              ...
              if (choice.response === 0) {
                // New project — send to renderer to navigate to dashboard with prefill
                ...
                // Import all files from folder into current project
                const importedFiles = []
                function readDirRecursive(dir, base = '') {
                  ...
                  for (const entry of entries) {
                    ...
                    if (entry.isDirectory()) {
                      readDirRecursive(abs, rel)
                    } else {
                      try {
                        const content = fs.readFileSync(abs, 'utf-8')
                        importedFiles.push({ filename: rel, content })
        ...
        },
        {
          ...
          click: async () => {
            const result = await dialog.showOpenDialog(mainWindow, {
              ...
              properties: ['openFile'],
              filters: [
                { name: 'Code Files', extensions: ['py', 'js', 'ts', 'tsx', 'jsx', 'html', 'css', 'json', 'md', 'txt', 'yaml', 'yml', 'sh', 'rs', 'go', 'sql'] },
        ...
        { type: 'separator' },
        {
          ...
          click: async () => {
            ...
            if (!result.canceled && result.filePath) {
              mainWindow?.webContents.send('menu:export-zip', { savePath: result.filePath })
            }
          }
        },
        { type: 'separator' },
        isMac ? { role: 'close' } : { label: 'Exit', accelerator: 'Alt+F4', role: 'quit' }
      ]
    },
    ...
    // ── Edit ────────────────────────────────────────────────────────────────
    {
      ...
      submenu: [
        ...
        },
        {
          ...
          click: () => { mainWindow?.webContents.send('menu:find-in-project') }
        },
      ]
    },
    ...
    // ── Window ──────────────────────────────────────────────────────────────
    {
      ...
      submenu: [
        ...
        },
        {
          ...
          click: () => { mainWindow?.webContents.send('menu:open-profile') }
        },
        { type: 'separator' },
        isMac ? { role: 'front' } : { role: 'togglefullscreen' },
      ]
    },
    ...
    // ── Help ────────────────────────────────────────────────────────────────
    {
      ...
      submenu: [
        {
          label: 'Documentation',
          click: () => { shell.openExternal('https://yantrik.com/docs') }
        },
        {
          label: 'Report a Bug',
          click: () => { shell.openExternal('https://github.com/yantrik/yantrik/issues') }
        },
        { type: 'separator' },
        {
          label: 'About Yantrik',
          click: () => {
            ...
            dialog.showMessageBox(mainWindow, {
              ...
              title: 'About Yantrik',
              message: 'Yantrik',
              detail: `Version: ${version}\nA collaborative AI-powered IDE.\n\nBuilt with Electron + FastAPI + React.`,
              buttons: ['OK'],
              icon: path.join(__dirname, '..', 'public', 'logo.png')
            })
          }
...
// ── File write IPC (for ZIP export etc.) ──────────────────────────────────────

ipcMain.handle('file:write-raw', (event, { filePath, buffer }) => {
  try {
